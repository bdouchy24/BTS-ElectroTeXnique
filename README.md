    ...
    - pour structurer son texte et les listes, se référer au cours déjà écrit et à la nomenclature approuvée tous ensemble avec entre autre (dès que vous éditez une nouvelle règle de nomenclature, consulter la team et l'écrire dans la liste suivante) :
    	...
    	- grands classiques de typographie avec entre autre :
        	...
        	- Guillemets français `\og \fg{}`
       		...
       		- Bien faire attention à `\oe{}`,  `\ier`, `\Dr`, `\ieme{}` les points de suspensions `\ldots...`
...
Rédiger les équations pour les exemples, détails de calcul, dans l'environnement `\begin{align}` ou `\begin{align*}` (pour éviter le référencement), dont dérive l'environnement `\begin{formule}[Titre_formule]` et qui permet d'aligner les équations sur le signe suivant `&` lors d'un retour à la ligne `\\` dans l'environnement. Pour les doublons nécessaires, utiliserl'environnement `\begin{formule*}[Titre_formule]` et y mettre un lien vers la formule déjà citée (attention aux majuscules/minuscules dans les instructions).

6. Rédiger les définitions dans l'environnement `\begin{definition}[Titre_définition]` qui va référencers les définitions dans une liste de définitions. Pour les doublons nécessaires, utiliserl'environnement `\begin{definition*}[Titre_définition]` et y mettre un lien vers la formule déjà citée (attention aux majuscules/minuscules dans les instructions).
