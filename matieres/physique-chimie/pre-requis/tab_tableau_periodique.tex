...
%% Legend
  \node at ($(La.north -| Fr.west) + (7.5em,-7.5em)$) [name=elementLegend, Element, fill=white] {\NaturalElementTextFormat{Z}{Masse}{Symbole}{Nom}{Configuration}};
    
%% Cores legend
  \node[name=CoreLegend, above of=elementLegend, CoreLegend] {Légende élément};

%% Diagram Title
  \node at ($(H.west -| Fe.north) + (-7em,0em)$) [name=diagramTitle, TitleLabel]
    {\underline{Tableau périodique des éléments chimiques}};
    
%% Improved Legend
	% First Block
	\draw[black, AlkaliMetalFill] ($(H.north -| V.west) + (-4em,-9.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Métaux alcalins}};
	\draw[black, AlkalineEarthMetalFill] ($(H.north -| V.west) + (-4em,-12.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Métaux alcalino-terreux}};
	\draw[black, MetalFill] ($(H.north -| V.west) + (-4em,-15.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Métaux}};
	\draw[black, MetalloidFill] ($(H.north -| V.west) + (-4em,-18.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Métalloides}};
	% Second Block
	\draw[black, NonmetalFill] ($(H.north -| Cr.east) + (2.5em,-9.5em)$)
 	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Non-métaux}};
	\draw[black, HalogenFill] ($(H.north -| Cr.east) + (2.5em,-12.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Halogènes}};
	\draw[black, NobleGasFill] ($(H.north -| Cr.east) + (2.5em,-15.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Gaz nobles}};
	\draw[black, LanthanideFill] ($(H.north -| Cr.east) + (2.5em,-18.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Lanthanides}};
	% Third Block
	\draw[black, ActinideFill] ($(H.north -| Fe.east) + (2.5em,-9.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Actinides}};
	\draw[black, PostTranMetalFill] ($(H.north -| Fe.east) + (2.5em,-12.5em)$)
	rectangle +(2em, 2em) node[right, yshift=-2ex]{\LARGE \textbf{Métaux de transition}};
