...
Rédiger les équations pour les exemples, détails de calcul, dans l'environnement `\begin{align}` ou `\begin{align*}` (pour éviter le référencement), dont dérive l'environnement `\begin{Theorem}` et qui permet d'aligner les équations sur le signe suivant `&` lors d'un retour à la ligne `\\` dans l'environnement. Pour les doublons nécessaires, utiliserl'environnement `\begin{theorem*}[Titre_définition]` et y mettre un lien vers la formule déjà citée (attention aux majuscules/minuscules dans les instructions).
